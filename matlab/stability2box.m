...
%     model = ['data2box/T1/problem2box_a' namealpha];
    model = 'dragon1-594523';
    % name of the files
    name_info = 'info';
...
%     M = loadMbin(model,'M0');
    M = loadMbin(model,'M50');
    % Compute clustering using stability criterion at different Markov times
    fprintf('Computing clustering...\n');
%     Markov_T = 10.^(1:0.02:3); % exponential sampling of Markov time is advised in the documentation
    Markov_T = 10.^(0:0.01:1);
    ...
    set(gcf,'Resize','off')
%     print(gcf,['/home/renaud/Documents/EPL/tfe/code/matlab/thuesday/stab_a' num2str(ind_alpha(i)) '.eps'],'-depsc2','-loose');
    print(gcf,[root 'out/' model '/stab_a' num2str(ind_alpha(i)) '.eps'],'-depsc2','-loose');
...
%         colormap(lines); % other colormap option
        ...
        xlabel('$y$');
        ylabel('$z$');
%         print(fig,['/home/renaud/Documents/EPL/tfe/code/matlab/thuesday/cluster_a' num2str(ind_alpha(i)) '_' num2str(N(ind)) '_.eps'],'-depsc2','-loose');
        print(fig,[root 'out/' model '/cluster_a' num2str(ind_alpha(i)) '_' num2str(N(ind)) '.eps'],'-depsc2','-loose');
